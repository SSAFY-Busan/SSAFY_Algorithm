...
# 따라서, 높이가 H보다 큰 나무는 H 위의 부분이 잘릴 것이고, 낮은 나무는 잘리지 않을 것이다. 예를 들어,
...
N, M = map(int, sys.stdin.readline().split())        # 나무 수, 필요한 높이
high = list(map(int, sys.stdin.readline().split()))  # 높이 리시트
high.sort()                             # 정렬한번하고
start = 0                               # 시작값 1
end = high[-1]                          # 끝값
ans = 0                                 # 정답
while start <= end:                     # 이분탐색
    mid = (start + end) // 2            # 중간값
    hap = 0                             # 나무 총합
    for i in high:                      # 높이 리스트 중에서
        if i >= mid:                    # 중간값보다 큰 값들 중에서
            hap += i - mid              # 합에 다더한다.
    if hap >= M:                        # 원하는 값 보다 크면
        start = mid + 1                 # 시작값을 중간값 + 1 로
        ans = mid                       # 정답 저장
    else:                               # 작으면
        end = mid - 1                   # 끝값을 중간값 - 1 로
print(ans)                              # 반복문 종료후 ans 뽑기
